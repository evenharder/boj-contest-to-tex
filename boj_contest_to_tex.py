...
class BOJContestProblem:
    """
    A data structure representing BOJ Contest Problem data.
    """
    ...
    def __init__(self):
        ...
        All members are initialized to None except _is_frozen (to False).
        """
    ...
    def is_frozen(self):
        """
        variable which determines whether the problem is freezed.
    ...
    def id(self):
        """
        BOJ Contest Problem Index,
        typically either A, B, C... or 1, 2, 3...
        """
        return self._id
    ...
    def ac_num(self):
        """
        The number of correct teams for the problem.
        Appends ' + ?' when frozen.
        """
        if self._is_frozen:
    ...
    def ac_ratio(self):
        """
        The correct ratio for the problem.
        Values are censored to question marks when frozen unless unsolved.
        Technically, this is not the exact AC ratio, but rather just a simple division between number of AC subs and total subs.
        This behavior is due to BOJ statistic's behavior.
        Also, spotboard does mark compile error as No, but BOJ statistics does not include it in the total submission.
        I hope no one nitpicks about this quirky discrepancy...
        """
        if self._is_frozen:
            return str("??.???\%")
        elif self._ac_num:
            if self._ac_ratio is None:
                self.ac_ratio = f"{100 * self._ac_num / self._total_sub:.3f}%"
    ...
    def ac_ratio(self, val):
        self._ac_ratio = str(val).replace("%", "\%")

    @property
    def total_sub(self):
        """
        The number of total submissions for the problem.
        Appends ' + ?' when frozen.
        """
        if self._is_frozen:
    ...
    def fs_time(self):
        """
        The earliest period when the problem is solved by any team.
        Values are censored to question marks when frozen unless unsolved.
        """
    ...
    def fs_teamname(self):
        """
        The teamname for the first solver.
        Values are censored to question marks when frozen unless unsolved.
        """
    ...
    def fs_members(self):
        """
        The team members of the first-solved team.
        Values are censored to question marks when frozen unless unsolved.
        """
    ...
    def fs_data(self):
        """
        A helpful description for the first-solved data.
        """
    ...
    def __repr__(self):
        """
        A simple represention of the problem metadata.
        """
...
First solve : {self.fs_teamname}, {self.fs_members}, {self.fs_time}"""
    ...
    def beamer_str(self, *, indent=" " * 4):
        """
        A TeX-style representation of the problem metadata.
        """
...
{indent}{indent}\item 출제자: \texttt{{\problemauthor{{\problemnum}}}}
{indent}\end{{itemize}}
"""
...
class SpotboardAnalyzer:
    ...
    def __init__(self, contest_json, runs_json):
        ...
        self.teams_id_map = {team["id"]: team["name"] for team in self.teams}
        self.team_ac = {team["id"]: set() for team in self.teams}

    def generate_run_data(
        self, *, problem_list=None, interval=5, contest_time=None, is_frozen=False
    ):
        """
    ...
    def generate_problem_data(self, prob_id, /, *, is_frozen):
        ...
        for cur_run in runs:
            ...
            if cur_run["result"] == "Yes":
                ...
                if is_first is False:
                    ...
                    fs_team = list(filter(lambda x: x["id"] == team_id, self.teams))[0]
        ...
        **kwargs
        ...
        for problem_list, filename in zip(problem_lists, filenames):
            self.export_run_data(
                self.generate_run_data(problem_list=problem_list, **kwargs), filename
            )
            problem_id = problem_list[0]
            if len(problem_list) == 1:
                self.generate_problem_data(problem_id, is_frozen=is_frozen)
...
class BOJStatisticsAnalyzer:
    ...
    def __init__(self, html_page):
        ...
        prob_nums = (
            index_r(arr, 1, contest_title, "합계")
            - index_r(arr, 1, contest_title, "문제 통계")
            - 1
        )
        ...
        while True:
            ...
            while (
                prob_index < len(self.boj_probs)
                and self.boj_probs[prob_index].id != cur_id
                or self.boj_probs[prob_index].name != cur_name
            ):
                prob_index += 1
